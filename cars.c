...
void init_intersection() {
	...
	isection.lanes[i].out_cars = NULL;           
        ...
        isection.lanes[i].head = 0; //first occupied spot
        isection.lanes[i].tail = 0; //next available spot
   ...
   struct lane *l = arg;
    ...
    do{
        ...
        if(cur_car != NULL)
        {
            ...
            j = 0;
            while(j < 4 && path[j] != 0){
                pthread_mutex_lock(&(isection.quad[path[j]-1]));
                j++;
            }
                     
            pthread_mutex_lock(&(isection.lanes[cur_car->out_dir].lock)); //lock of the outbound lane
...
int *compute_path(enum direction in_dir, enum direction out_dir) {
    ...
    if(path == NULL)
    {
        fprintf(stderr, "don't have memory\n");
    }	
    if(in_dir == NORTH)
    {
        if(out_dir == NORTH)
